...
@Component({
  selector: 'app-kursus-detail',
  templateUrl: './course-detail.page.html',
  styleUrls: ['./course-detail.page.scss'],
  standalone: true,
  imports: [IonicModule, CommonModule, HeaderComponent, RouterModule],
})
export class KursusDetailPage implements OnInit, ViewWillEnter {
  ...
  get isFree(): boolean {
    return !this.course?.base_price || Number(this.course.base_price) <= 0;
  }

  doPurchase() {
    // Kursus berbayar wajib lewat checkout (iPaymu); gratis daftar langsung.
    if (!this.isFree) {
  ...
  goToMaterial(mat: any) {
    ...
    this.navCtrl.navigateForward(`/tabs/kursus/${this.course.id}/materi/${mat.id}`);
  }
